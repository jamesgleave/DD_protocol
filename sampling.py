...
def train_valid_test(file_name):
    ...
    train_set = open(protein + "/iteration_" + str(n_it) + "/train_set.txt", 'a')
    test_set = open(protein + "/iteration_" + str(n_it) + "/test_set.txt", 'a')
    valid_set = open(protein + "/iteration_" + str(n_it) + "/valid_set.txt", 'a')
    ...
    print(f'train/valid/test counts: {len(train_ind_dict)}, {len(valid_ind_dict)}, {len(test_ind_dict)}')
...
def sampling(protein, n_it, data_directory, tot_process, tr_sz, vl_sz, rt_sz):
    if not os.path.exists(protein+"/iteration_"+str(n_it)):
        os.mkdir(protein+"/iteration_"+str(n_it))

    f_names = []
    for f in glob.glob(data_directory+'/chunk*.txt'):
        f_names.append(f)
...
sampling(protein, n_it, data_directory, tot_process, tr_sz, vl_sz, rt_sz)