...
def extract_morgan(file_name):
    ...
    test = {}
    valid = {}
    with open(protein + "/iteration_" + str(n_it) + "/train_set.txt", 'r') as ref:
        for line in ref:
            train[line.rstrip()] = 0
    with open(protein + "/iteration_" + str(n_it) + "/valid_set.txt", 'r') as ref:
        for line in ref:
            valid[line.rstrip()] = 0
    with open(protein + "/iteration_" + str(n_it) + "/test_set.txt", 'r') as ref:
        for line in ref:
            test[line.rstrip()] = 0

    # for file_name in file_names:
    ref1 = open(protein + '/iteration_' + str(n_it) + '/morgan/' + 'train_' + file_name.split('/')[-1], 'w')
    ref2 = open(protein + '/iteration_' + str(n_it) + '/morgan/' + 'valid_' + file_name.split('/')[-1], 'w')
    ref3 = open(protein + '/iteration_' + str(n_it) + '/morgan/' + 'test_' + file_name.split('/')[-1],'w')
...
def extracting_morgan(protein, n_it, morgan_directory, tot_process):
    try:
        os.mkdir(protein + '/iteration_' + str(n_it) + '/morgan')
    except:
        pass
    ...
    for type_to in ['train', 'valid', 'test']:
        #t = time.time()
        files = []
        for f in glob.glob(protein + '/iteration_' + str(n_it) + '/morgan/' + type_to + '*'):
            files.append(f)
        ...
        with open(protein + '/iteration_' + str(n_it) + '/morgan/' + type_to + '_morgan_1024.csv',
                  'w') as ref:
    ...
    f_names = []
    for f in glob.glob(protein + '/iteration_' + str(n_it) + '/morgan/*morgan*'):
        f_names.append(f)
...
print(" - Morgan Directory:", morgan_directory)

extracting_morgan(protein, n_it, morgan_directory, tot_process)